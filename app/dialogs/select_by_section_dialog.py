...
class SelectByFrameSectionDialog(QDialog):
    ...
    def _refresh_overlay(self):
        ...
        if not hasattr(mw, 'selected_area_ids'): 
            mw.selected_area_ids = []
            
        for cvs in [mw.canvas, mw.canvas2]:
            cvs.update_selection_overlay(mw.selected_ids, mw.selected_node_ids, mw.selected_area_ids)
            
        n_frames = len(mw.selected_ids)
        n_areas = len(mw.selected_area_ids)
        
        parts = []
        if n_frames: parts.append(f"{n_frames} Frame{'s' if n_frames != 1 else ''}")
        if n_areas:  parts.append(f"{n_areas} Area{'s' if n_areas != 1 else ''}")
        parts.append(f"{len(mw.selected_node_ids)} Joints")
        
        mw.status.showMessage(f"Selected: {', '.join(parts)}")
...
class SelectByAreaSectionDialog(QDialog):
    def __init__(self, main_window, mode="select"):
        super().__init__(main_window)
        self.main_window = main_window
        self.mode = mode

        title = "Select by Area Section Properties" if mode == "select"\
                else "Deselect by Area Section Properties"
        self.setWindowTitle(title)
        self.resize(320, 420)
        self.setModal(False)
        self.setWindowFlags(Qt.WindowType.Window | Qt.WindowType.WindowStaysOnTopHint)
        ...
        list_grp = QGroupBox("Area Section Properties")
        list_lay = QVBoxLayout()
        self.list_widget = QListWidget()
        self.list_widget.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        list_lay.addWidget(self.list_widget)
    ...
    def _check_selection_changed(self):
        """Repopulate if selected_area_ids changed externally."""
        current = set(getattr(self.main_window, 'selected_area_ids', []))
        if current != self._last_snapshot:
            self._last_snapshot = current
            self._populate()
    ...
    def _populate(self):
        ...
        if not model or not hasattr(model, 'area_elements'):
            return

        if self.mode == "deselect":
            selected_ids = set(getattr(self.main_window, 'selected_area_ids', []))
            elems = {aid: model.area_elements[aid] for aid in selected_ids
                     if aid in model.area_elements}
        else:
            elems = model.area_elements
    ...
    def _elem_ids_for_sections(self, names):
        ...
        if not model or not hasattr(model, 'area_elements'):
            return []
        return [
            aid for aid, elem in model.area_elements.items()
            if (getattr(getattr(elem, 'section', None), 'name', None) or "None") in names
        ]
    ...
    def _do_select(self):
        ...
        for aid in self._elem_ids_for_sections(names):
    ...
    def _do_deselect(self):
        ...
        to_remove = set(self._elem_ids_for_sections(names))
        mw.selected_area_ids = [aid for aid in mw.selected_area_ids if aid not in to_remove]
        self._refresh_overlay()
